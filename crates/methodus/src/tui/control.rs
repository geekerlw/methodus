...
const SLASH_COMMANDS: &[SlashCmd] = &[
    ...
    SlashCmd { name: "graph", aliases: &[], summary: "Open the current knowledge graph" },
...
#[derive(Clone, Copy, PartialEq, Eq)]
enum ConfirmAction { Commit, Reject, MarkTeam, Deprecate, Revalidate, Merge }
...
impl App {
    fn new(engine: Engine) -> Self {
        ...
        Self {
            ...
            experience_filter: String::new(), review_filter: String::new(), merge_filter: String::new(),
    ...
    fn panel(&mut self, view: View) { self.view = view; self.selected = 0; self.confirmation = None; self.refresh(); }
    ...
    fn retain_confirmation_for(&mut self, code: KeyCode) {
        let keep = matches!((self.confirmation, code), (Some(ConfirmAction::Commit), KeyCode::Char('c')) | (Some(ConfirmAction::Reject), KeyCode::Char('r')) | (Some(ConfirmAction::MarkTeam), KeyCode::Char('t')) | (Some(ConfirmAction::Deprecate), KeyCode::Char('x')) | (Some(ConfirmAction::Revalidate), KeyCode::Char('v')) | (Some(ConfirmAction::Merge), KeyCode::Enter));
...
fn handle_key(terminal: &mut Terminal<CrosstermBackend<std::io::Stdout>>, app: &mut App, key: KeyEvent) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    match app.view {
        ...
        View::KnowledgeDetail | View::ExperienceDetail | View::ReviewDetail => match key.code { KeyCode::Esc | KeyCode::Char('q') => { app.view = app.detail_return; app.knowledge_scroll = 0; }, KeyCode::Char('g') => open_graph(app, app.detail_return), KeyCode::Char('e') => edit_current_node(terminal, app)?, KeyCode::Up => app.knowledge_scroll = app.knowledge_scroll.saturating_sub(1), KeyCode::Down => app.knowledge_scroll = app.knowledge_scroll.saturating_add(1), KeyCode::PageUp => app.knowledge_scroll = app.knowledge_scroll.saturating_sub(10), KeyCode::PageDown => app.knowledge_scroll = app.knowledge_scroll.saturating_add(10), KeyCode::Home => app.knowledge_scroll = 0, KeyCode::Char('c') if app.view == View::ReviewDetail => commit_candidate(app), KeyCode::Char('r') if app.view == View::ReviewDetail => reject_candidate(app), KeyCode::Char('t') if app.view == View::ReviewDetail => mark_candidate_team(app), KeyCode::Char('m') if app.view == View::ReviewDetail => open_merge_target(app), KeyCode::Char('x') => deprecate_node(app), KeyCode::Char('v') => revalidate_node(app), _ => {} },
...
fn handle_list_key(app: &mut App, key: KeyEvent, ctrl: bool) {
    ...
    match key.code {
        KeyCode::Esc if !filter.is_empty() => { filter.clear(); app.selected = 0; app.status = "Filter cleared".into(); }, KeyCode::Esc => app.view = View::Chat,
...
fn submit_chat(terminal: &mut Terminal<CrosstermBackend<std::io::Stdout>>, app: &mut App) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if app.learning_goal.is_none() { app.learning_goal = Some(text.clone()); }
    app.say(format!("You: {text}")); app.status = format!("{} is thinking…", runtime_label(&app.runtime)); terminal.draw(|frame| draw(frame, app))?;
    let runtime = app.runtime.clone(); let permission_mode = app.permission_mode.clone(); let existing_sid = app.learning_executor_sid.clone(); let existing_session = app.learning_session_id.clone();
    let result = tokio::task::block_in_place(|| tokio::runtime::Handle::current().block_on(async {
        let (handle, mut events) = match (existing_sid.as_deref(), existing_session.as_deref()) { (Some(sid), Some(session_id)) => app.engine.continue_learning(&runtime, &permission_mode, sid, session_id, &text).await?, _ => app.engine.start_learning(Some(&runtime), &permission_mode, &text).await? };
        let mut answer = String::new(); let mut returned_sid = handle.executor_sid.clone(); let run_id = handle.session_id.clone(); let _ = app.engine.record_learning_event(&run_id, "user", &text);
        while let Some(event) = events.recv().await { match event {
            RuntimeEvent::AssistantText { text } if !text.trim().is_empty() => { if !answer.is_empty() { answer.push('\n'); } answer.push_str(&text); },
            RuntimeEvent::Result { text, session_id, is_error, .. } => { if session_id.is_some() { returned_sid = session_id; } if is_error { let _ = app.engine.record_learning_event(&run_id, "runtime", &format!("result error: {text}")); return Err(methodus_core::CoreError::Other(text)); } if !text.trim().is_empty() { answer = text; } break; },
            RuntimeEvent::SessionStarted { session_id } => { returned_sid = Some(session_id.clone()); let _ = app.engine.record_learning_event(&run_id, "runtime", &format!("session started: {session_id}")); },
            RuntimeEvent::Thinking { text } => { let _ = app.engine.record_learning_event(&run_id, "runtime", &format!("thinking: {}", text.chars().take(2000).collect::<String>())); },
            RuntimeEvent::ToolCallStarted { name, input, .. } => { let _ = app.engine.record_learning_event(&run_id, "runtime", &format!("tool started {name}: {input}")); },
            RuntimeEvent::ToolCallCompleted { output, exit_code, .. } => { let _ = app.engine.record_learning_event(&run_id, "runtime", &format!("tool completed ({exit_code:?}): {output}")); },
            RuntimeEvent::ApprovalRequested { tool_name, .. } => { let _ = app.engine.record_learning_event(&run_id, "runtime", &format!("approval requested: {tool_name}")); },
            RuntimeEvent::TurnCompleted { stop_reason } => { let _ = app.engine.record_learning_event(&run_id, "runtime", &format!("turn completed: {stop_reason:?}")); },
            RuntimeEvent::Error { message } => { let _ = app.engine.record_learning_event(&run_id, "runtime", &format!("error: {message}")); return Err(methodus_core::CoreError::Other(message)); },
            _ => {}
        } }
        Ok::<_, methodus_core::CoreError>((handle.session_id, returned_sid, answer))
    }));
    match result {
        Ok((session_id, executor_sid, answer)) => {
            app.learning_session_id = Some(session_id);
            app.learning_executor_sid = executor_sid;
            if answer.trim().is_empty() {
                app.say("Methodus: The learning runtime returned no displayable content.".to_string());
                app.status = "Learn · ready for a follow-up".into();
                if let Some(run_id) = app.learning_session_id.as_deref() { let _ = app.engine.record_learning_event(run_id, "assistant", ""); let _ = app.engine.mark_learning_status(run_id, "awaiting_input"); }
            } else {
                app.say(format!("Methodus: {answer}"));
                if let Some(run_id) = app.learning_session_id.as_deref() { let _ = app.engine.record_learning_event(run_id, "assistant", &answer); }
                if looks_like_candidate_set(&answer) {
                    let goal = app.learning_goal.as_deref().unwrap_or(text.as_str());
                    let run_id = app.learning_session_id.as_deref().unwrap_or("learn_untracked");
                    match app.engine.record_learning_output_for_run(run_id, goal, &answer) {
                        Ok(candidate_ids) => { app.refresh(); app.status = format!("Learn · generated {} candidate(s) · type /review to review", candidate_ids.len()); }
                        Err(error) => app.status = format!("Learn returned, but candidates could not be stored: {error}"),
                    }
                } else {
                    app.status = "Learn · the runtime is asking for clarification; reply to continue".into();
                    if let Some(run_id) = app.learning_session_id.as_deref() { let _ = app.engine.mark_learning_status(run_id, "awaiting_input"); }
                }
            }
        }
        Err(error) => { app.say(format!("Methodus: Learning runtime failed: {error}")); if let Some(run_id) = app.learning_session_id.as_deref() { let _ = app.engine.mark_learning_status(run_id, "failed"); } app.status = "Learn runtime unavailable; existing graph panels remain available".into(); }
    }
    Ok(())
}

fn command(terminal: &mut Terminal<CrosstermBackend<std::io::Stdout>>, app: &mut App) -> Result<(), Box<dyn std::error::Error + Send + Sync>> { let matches = matching_slash(&app.input); let Some(command) = matches.get(app.slash_sel).copied() else { app.status = "Unknown command; type / to browse available commands".into(); return Ok(()); }; let rest = slash_rest(&app.input); app.clear_input(); match command.name { "knowledge" => app.panel(View::Knowledge), "graph" => { app.panel(View::Knowledge); if app.items_len() > 0 { open_graph(app, View::Knowledge); } else { app.status = "The graph is empty; capture a knowledge note in Learn first".into(); } }, "method" => app.panel(View::Method), "experience" => app.panel(View::Experience), "review" => app.panel(View::Review), "team" => app.panel(View::Team), "health" => app.panel(View::Health), "runtime" => { if rest.is_empty() { open_runtime_picker(app); } else { select_runtime(app, &rest); } }, "open" => open_path(terminal, app, &rest)?, "help" => app.panel(View::Help), "new" => { close_current_learning(app); if !rest.is_empty() { app.input = rest; app.input_cursor = app.input.len(); return submit_chat(terminal, app); } }, "quit" => app.quit = true, _ => app.status = format!("Unknown command /{}", command.name) } Ok(()) }
...
fn deprecate_node(app: &mut App) {
    ...
    if !confirm_action(app, ConfirmAction::Deprecate, &format!("Confirm archive of '{}'", node.title)) { return; }
    match app.engine.deprecate_graph_node(&node.id, "deprecated from Methodus TUI") {
        Ok(()) => { app.status = format!("Archived {}", node.title); app.view = app.detail_return; app.refresh(); }
        Err(error) => app.status = format!("Archive failed: {error}"),
    }
}
...
fn open_merge_target(app: &mut App) { let Some(candidate) = app.nodes.iter().find(|node| node.id == app.detail_node_id) else { app.status = "No candidate available for merging".into(); return; }; if candidate.node_type != "knowledge" || candidate.status.as_deref() != Some("candidate") { app.status = "Only knowledge candidates can be merged".into(); return; } app.merge_candidate_id = Some(candidate.id.clone()); app.merge_filter.clear(); app.selected = 0; app.view = View::MergeTarget; app.status = format!("Select the committed knowledge target for '{}'; press Enter twice to confirm", candidate.title); }
...
fn draw_footer(frame: &mut Frame, area: Rect, app: &App, theme: &Theme) { let style = if app.status.contains("failed") || app.status.contains("Failed") || app.status.contains("Could not") { Style::default().fg(theme.error).add_modifier(Modifier::BOLD) } else if app.status.starts_with("Archived") || app.status.starts_with("Revalidated") || app.status.starts_with("Promoted") || app.status.starts_with("Rejected") || app.status.starts_with("Merged") || app.status.starts_with("Opened") || app.status.starts_with("Reloaded") || app.status.starts_with("Publish plan written") { Style::default().fg(theme.success) } else { Style::default().fg(theme.info) }; frame.render_widget(Paragraph::new(app.status.as_str()).style(style), area); }
...
fn draw_nodes(frame: &mut Frame, area: Rect, app: &App, theme: &Theme) { let nodes = nodes_for_view(app); let cols = Layout::default().direction(Direction::Horizontal).constraints([Constraint::Percentage(44), Constraint::Percentage(56)]).split(area); let items = nodes.iter().map(|n| ListItem::new(Line::from(vec![Span::styled(format!("{}  ", n.status.as_deref().unwrap_or("—")), theme.dim()), Span::styled(&n.title, theme.text())]))).collect::<Vec<_>>(); let mut state = ListState::default(); if !nodes.is_empty() { state.select(Some(app.selected)); } frame.render_stateful_widget(List::new(items).highlight_style(theme.selected()).highlight_symbol("› "), cols[0], &mut state); let filter = active_filter(app); let detail = nodes.get(app.selected).map(|node| format!("{}{}\n\n{}\n\nstatus      {}\nvisibility  {}\ntags        {}\nscope       {}\nconfidence  {}\npath        {}\n\n{}", if !filter.is_empty() { format!("filter      {}\n\n", filter) } else { String::new() }, node.title, node.summary.as_deref().unwrap_or("No summary available."), node.status.as_deref().unwrap_or("—"), node.visibility, if node.tags.is_empty() { "—".into() } else { node.tags.join(", ") }, node.scope.as_deref().unwrap_or("—"), node.confidence.map(|v| format!("{v:.2}")).unwrap_or_else(|| "—".into()), node.path, if app.view == View::Review { "c twice approve · t twice team · m twice merge · r twice reject" } else if app.view == View::MergeTarget { "Enter merges the candidate into this knowledge. Esc cancels." } else { "Enter reads the complete Markdown note · g opens graph" })).unwrap_or_else(|| if !filter.is_empty() { format!("No matches for ‘{}’.\n\nType to change the filter or press Esc to clear it.", filter) } else { "Nothing here yet.".into() }); frame.render_widget(Paragraph::new(detail).style(theme.overlay_text()).wrap(Wrap { trim: true }).block(Block::default().borders(Borders::LEFT).border_style(theme.border(false))), cols[1]); }
fn draw_help(frame: &mut Frame, area: Rect, theme: &Theme) { let lines = vec![Line::from(Span::styled("Commands", theme.accent())), Line::from("Plain input   Start or continue the current Learn"), Line::from("/new          Close the current context and start a new Learn"), Line::from("/knowledge    Browse, filter, and read the knowledge graph"), Line::from("/method       Browse reusable working methods"), Line::from("/experience   Browse validated task experience"), Line::from("/review       Review candidates and approve promotion"), Line::from("/team         Inspect the Personal / Team roots"), Line::from("/health       Check sources, indexes, and connector status"), Line::from("/runtime      Open the runtime picker; /runtime codex also works"), Line::from("/open         Open the current node or a path"), Line::from("/quit         Exit Methodus; q does not quit"), Line::default(), Line::from(Span::styled("Navigation", theme.accent())), Line::from("/             Open the command palette; ↑↓ select; Tab complete; Enter run"), Line::from("Lists         Type to filter; Enter read; g open graph; Esc clear/back"), Line::from("Review        Enter read; c approve; t team; m merge; r reject"), Line::from("Composer      Enter send; Shift+Enter newline; Shift+Tab cycle permission"), Line::from("Exit          Ctrl+C clears input; press it again within three seconds to quit")]; frame.render_widget(Paragraph::new(lines).style(theme.overlay_text()).wrap(Wrap { trim: false }), area); }
...
fn draw_detail(frame: &mut Frame, area: Rect, app: &App, theme: &Theme) { let Some(node) = app.nodes.iter().find(|node| node.id == app.detail_node_id) else { return; }; let kind = match app.view { View::ExperienceDetail => "experience", View::ReviewDetail => "review", View::KnowledgeDetail if node.node_type == "method" => "method", _ => "knowledge" }; let action = if app.view == View::ReviewDetail { "c×2 approve · t×2 team · m×2 merge · r×2 reject · e edit · x×2 archive · v×2 revalidate" } else { "↑↓ / PgUp/PgDn scroll · g graph · e edit · x×2 archive · v×2 revalidate · Esc back" }; let block = Block::default().borders(Borders::ALL).border_type(BorderType::Rounded).border_style(theme.border(true)).title(Line::from(vec![Span::styled(format!(" {} · {} ", kind, node.title), theme.accent()), Span::styled(action, theme.dim())])); let inner = block.inner(area); frame.render_widget(block, area); frame.render_widget(Paragraph::new(app.knowledge_content.as_str()).style(theme.text()).wrap(Wrap { trim: false }).scroll((app.knowledge_scroll, 0)), inner); }
...
fn open_graph(app: &mut App, return_view: View) { let nodes = if return_view == View::Knowledge { filtered_nodes(app.engine.home(), &app.nodes, "knowledge", None, &app.knowledge_filter) } else if return_view == View::Method { filtered_nodes(app.engine.home(), &app.nodes, "method", None, &app.method_filter) } else { app.nodes.iter().filter(|node| node.id == app.detail_node_id).cloned().collect() }; let Some(focus) = nodes.get(app.selected).cloned().or_else(|| app.nodes.iter().find(|node| node.id == app.detail_node_id).cloned()) else { app.status = "No node is available to expand".into(); return; }; let edges = match app.engine.graph_edges_for(&focus.id) { Ok(edges) => edges, Err(error) => { app.status = format!("Could not read graph edges: {error}"); return; } }; let mut ids = vec![focus.id.clone()]; for edge in &edges { let other = if edge.from_id == focus.id { &edge.to_id } else { &edge.from_id }; if !ids.contains(other) { ids.push(other.clone()); } } app.graph_nodes = ids.iter().filter_map(|id| app.nodes.iter().find(|node| &node.id == id).cloned()).collect(); app.graph_edges = edges; app.graph_selected = 0; app.graph_return = return_view; app.view = View::KnowledgeGraph; app.status = format!("Graph expanded: {} node(s) · ↑↓ select · Enter open · Esc back", app.graph_nodes.len()); }
...
fn nodes_for_view(app: &App) -> Vec<GraphNode> { match app.view { View::Knowledge => filtered_nodes(app.engine.home(), &app.nodes, "knowledge", None, &app.knowledge_filter), View::Method => filtered_nodes(app.engine.home(), &app.nodes, "method", None, &app.method_filter), View::Experience => filtered_nodes(app.engine.home(), &app.nodes, "experience", None, &app.experience_filter), View::Review => filtered_nodes(app.engine.home(), &app.nodes, "", Some("candidate"), &app.review_filter), View::MergeTarget => filtered_nodes(app.engine.home(), &app.nodes, "knowledge", Some("committed"), &app.merge_filter), _ => Vec::new() } }
...
fn looks_like_candidate_set(answer: &str) -> bool {
    let lower = answer.to_lowercase();
    lower.contains("candidateset") || lower.contains("candidate set") && lower.contains("knowledge") || lower.contains("\"candidates\"") || lower.contains("```json") && lower.contains("candidates")
}
...
mod tests {
    use super::*;
    #[test] fn slash_palette_exposes_management_commands() { let names = matching_slash("/").into_iter().map(|command| command.name).collect::<Vec<_>>(); for required in ["knowledge", "graph", "method", "experience", "team", "health", "runtime", "review", "new", "open", "quit"] { assert!(names.contains(&required)); } assert!(!names.contains(&"learn")); assert_eq!(matching_slash("/exit")[0].name, "quit"); }
    ...
    #[test] fn only_synthesis_responses_create_candidates() { assert!(!looks_like_candidate_set("First confirm the device model and log range.")); assert!(looks_like_candidate_set("```json\n{\"candidates\":[]}\n```")); assert!(looks_like_candidate_set("candidate set: knowledge")); }
